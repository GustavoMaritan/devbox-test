...
class Test extends Uteis {
    ...
    async start(messages) {
        ...
        this._listMessage(`----------- TESTE => ${this.title} -----------`, this._colors.magenta);
        //this._listMessage(`-----------------------------------`, this._colors.white);
        ...
        this._listMessage(`**                                              **`, this._colors.white);
        this._listMessage(`**             Métodos testados - ${this._padLeft(this.stats.events)}           **`, this._colors.blue);
        ...
        this._listMessage(`**************************************************`, this._colors.white);
        this._listMessage(``, this._colors.white);
        this._listMessage(`----------- FINALIZADO ${this.title} -----------------------------`, this._colors.magenta);
        this._listMessage(``, this._colors.white);
        return this;
    }
    ...
    $urlPrepare(config) {
        let _this = this;
        return () => {
            config.headers = this._assign(config.headers || {}, this.headers || {});
    ...
    _finish(error, id, content) {
        ...
        if (error) {
            ...
            this.stats.errors++;
            this._listMessageExec();
            this._listMessage(`Teste ${exec.config.title || ''} finalizado com erro.`, this._colors.red);

            if (exec.config.printError) {
                this._listMessage(`Error object.`, this._colors.yellow);
                this._listMessage(this.printObject(error), this._colors.yellow);
            }
            this._listMessage(`-----------------------------------`, this._colors.white);
            ...
            this._listMessageExec();
            this._listMessage(`Teste ${exec.config.title || ''} finalizado com sucesso.`, this._colors.green);
