...
class Uteis {
    ...
    set logs(value) {
        !Array.isArray(value)
            ...
            : value.map(x => this._logs.push(x));
    }

    get logsString() {
        let m = this._logs.map(x => {
            if (x.many) {
    ...
    _setMsgPrintStats(message, cor) {
        return _setMsgIsValid(this.config.printStats, message, cor)
    }

    _setMsgPrintCtrlStats(message, cor) {
        return _setMsgIsValid(this.config.printCtrlStats, message, cor)
    }

    _setMsgPrintError(printError, message, cor) {
        return _setMsgIsValid(printError, message, cor)
    }

    _setMsgPrintSuccess(printSuccess, message, cor) {
        return _setMsgIsValid(printSuccess, message, cor)
    }
